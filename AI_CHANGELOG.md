...
`eca99bb` corrected the canonical clavicle's rest angle for the curl and cost 8 failures elsewhere, reported at the time and deferred. All eight are now closed at the right level of the escalation ladder, each from measurement:

**Push-up (3 failures) — exercise definition.** The arm chain moved 35 mm posterior; a push-up body is prone, so posterior is *up*, and the shoulder rose 33.7 mm off the floor. The arm then had to span 565.3 mm to reach a hand fixed on the floor against the 560.0 mm it has — it locked out, ran out of reach by 5.3 mm, and with no bend left the elbow could not sit out (flare 110.3 → 57.3 mm, through the 70 mm the rule requires). Pitch and root are one parameter, not three: the toe sits at a fixed offset in the root's frame, so `root = toeWorld − R(pitch)·toeLocal`, and evaluating that at 74°/84° reproduces the shipped roots to 0.01 mm, which confirmed the relation before it was used. Re-derived at **75.59°/85.54°** the push-up is numerically the pre-correction exercise: shoulder 572.7 mm at the top and 353.0 mm at the bottom against 572.7 and 353.0 before, stroke 219.7 mm, toes on the floor, hand placement and elbow poles untouched. The two-hand grip-socket gate was a cascade from this and cleared with it.

**Procedural body and écorché (3 failures) — shared system.** The built-in body is a baked asset with absolute vertices, so the bone moved *inside* it: measured, `upperarm_l` ended up **38.74 mm** behind the middle of its own arm surface, where it had been within 3.74 mm. This is the same problem `SHOULDER_WIDENING` already solves in x, and it takes the same remedy — a new `SHOULDER_SETBACK` constant applied through the existing graph-distance band at build time, so the encoded arrays stay as authored and one constant drives rig and surface. Registration is back to 5.56 mm. Giving the setback a wider band of its own was swept from 3 to 18 hops and rejected: it buys little (6.34× at 2 hops against 5.91× at 12) and every width past 2 breaks something the narrow band leaves alone — the arm's declared limits at 3–4, the neck-to-shoulder folds at 6, the nape ledge and head-turn strain by 12.

**Deltoid belly (1 failure) — muscle anchor.** The anchor never stopped tracking the bone: it sits 24.17 mm off the clavicle's axis before and after, and swung back 24.9 mm with the girdle exactly as it should. What changed is what it swung into — the chest is bound to `spine_03` and rightly did not move, so the belly's anterior face came to lie 2.187 mm outside the pectoral surface at the curl's peak against 1.344 mm inside it before. Local z 0.022 → 0.016 puts it back (worst −1.020 mm) with the flexion action well clear of the agonist floor (0.829 against the 0.98 required).

**One regression remains open and is recorded as one.** The shoulder strain sweep's worst case went 5.215× before the correction, to 6.771× with the bones moved and the surface left behind, to **6.336×** now. The residue is a single 14.7 mm edge across the front of the armpit, at both arms in 165° of forward flexion — a pose no exercise reaches. Every other pose in the sweep is at or below 2.664×, within noise of its pre-correction value. Rather than raise the one ceiling, the test now carries two: the envelope below full forward flexion is held at **2.8×**, far tighter than the 5.5× it used to carry, and the one regressed pose carries its own at 6.4×. Closing it properly means re-baking the surface against the corrected rig.

**New: `src/rig/rigRegression.test.ts`**, the gate whose absence let this land as a diff instead of a report. It ties `SHOULDER_SETBACK`/`SHOULDER_WIDENING` back to the authored bone literals, asserts the baked surface stays registered with the bone inside it in both axes, and runs the whole exercise library through generate/validate with a per-exercise PASS/FAIL report naming the rule and the measured value. Both regressions were re-introduced to confirm it fails on them: it reports `arm surface is 38.74 mm off its bone in z` and `Push-Up  technique 2, unreachable 22 (worst 5.56 mm), contact drift 5.56 mm hand_l at 2.63s`.
...
Two faults were stacked in the broken candidate. `bindRetarget` takes each bone's rest frame from its head to its mapped child's head and measures the correction against the bone's own stored bind orientation — so translating `DEF-upper_arm.*` moved the **clavicle's tail** while leaving the clavicle's stored orientation behind, and the correction absorbed the difference. Stage 1's forearm tool was safe only because it translated along a bone axis, leaving every direction unchanged. Fixed by rotating the clavicle through exactly the angle its tail moved (10.147°, symmetric, head held) so stored orientation and derived frame stay in step. Underneath that sat a matrix-order error: the helper multiplies row-major while glTF stores column-major, so `mul(A,B)` is B·A and a world transform must be applied as `mul(W,X)` — both of mine were reversed, verified against an independent reader. The tool now re-reads its own output and checks achieved placement: clavicle heads held at 0.00 mm, arm chain purely lateral at 38.82 mm both sides, **exact to 6.8e-13 mm**.

Equivalence, stated functionally: import height and scale identical to Stage 1 (2.017579 / 0.867376, against the broken candidate's 2.261510 / 0.773819 — a 10.8% global error); **bones off the arm move 0.0000 mm** under the same canonical poses across curl and press; grip contact on the widened rig is **identical to locked Phase 1 to the digit** (fingers −0.46/−0.36/−0.22/−0.11, zero inside; thumb +1.18; palm −2.54; wrap 252°); and curl Bottom/Return thigh clearance **improves to +4.68/+4.79 mm** from +1.81/+1.90, because the wider shoulder carries the weight further from the leg.

All the previously validated Stage 2 work re-applied from the record with no value re-derived: canonical rig widening, the press/push-up/pull-up envelopes, the rack sockets, the measured hang height, the 2-hop graph-distance surface ramp (shoulder edge strains still better than unshifted, 5.215 vs 5.308 and 0.1580 vs 0.1324), the deltoid origins, the pec/lat insertions and the deltoid-width envelope. All four exercises validate clean. Reference overlay on the corrected candidate: shoulders **−4.17% → −0.29%** of figure height, about 5 mm on a 520 mm span and inside every other landmark's error. The earlier exact 0.00 came from the broken candidate, whose silhouette was not a pose the rig can produce.

**Open item:** `pectoralis_l` reaches 7.84 mm outside the containment skin in the push-up against a 7 mm allowance (latissimus 7.34 mm). Diagnosed: both exit nearest `spine_03`, the chest's lateral edge, into the band the widened arm vacated between a chest that must not widen and a deltoid section that moved out with the humerus — a gap in the containment proxy, not added mass. Every permitted lever was measured: insertions onto the shaft fixed the press case, lateral origins took it 9.86 → 7.84, `taper` only migrates the violation between poses and muscles (and makes the latissimus worse), `flatten` has no effect on the probed depth, explicit `outward` moves it under 0.3 mm, and an extra interpolated clavicle ring changes the figure by zero digits because the nearest section is the chest. What remains is widening the chest, scaling a belly, or opening the containment skin's armpit coverage — the first two are forbidden and the third runs close to "do not inflate deltoid mass", so it is put to the user rather than taken unilaterally. Suite 297 passed / 1 skipped / 1 failed; Phases 3–5 not started.

### Claude — 2026-09-17 — Phase 2: Stage 2 executed under both authorisations, then reverted on a character-candidate blocker

Result in `docs/REFERENCE_BODY_PHASE2_EXECUTION.md`. Phase 1 untouched, nothing promoted or merged, tree green at 298 passed / 1 skipped / 0 failed.

Both decisions were exercised and nearly everything validated. Canonical rig widened by `SHOULDER_WIDENING = 0.01924 × RIG_HEIGHT` (33.67 mm per side) over the clavicle tail and everything outboard, nothing scaled: deltoid-to-deltoid went 433 → 500 mm against a reference of about 520 mm. **All four exercises validated clean** after re-deriving only genuinely shoulder-relative rules — press and push-up width envelopes, pull-up grip plus the rack sockets (±0.24 → ±0.274; the bar is 1.3 m wide with uprights at ±0.62, so no physical conflict), and the pull-up hang height, which was measured rather than guessed: the longer clavicle lifts the shoulder 8.1 mm at the hang, cutting shoulder-to-bar to 549 mm against a 560 mm arm and bending the elbow to 22.8°, so dropping the root the same 8.1 mm restored 11.86° against 12.06° before Stage 2. Push-up's `body_line` was the cleanest case: its sagittal deviation measures **0.0000 at every frame**, so the whole 0.0669 is the constant lateral fact that a shoulder is wider than a hip, and carrying the widening into the tolerance leaves the sagittal margin at 26.8 mm — exactly what it was — policing sag and pike identically.

Procedural anatomy followed the rig without widening the torso. The baked surface had to move (upper-arm girth otherwise collapsed 97 → 30 mm, since girth is read beyond the shoulder head), and the transition had to span several edges: the shift is 33.7 mm against a 7 mm median edge, so a mask going 0 → 1 across one edge inverts it, measured at 5.5% of rest length against a 10% floor. Averaging the weight field made it worse and bled into the neck; a 2-hop graph-distance ramp gave **worst 5.215 and tightest 0.1580, both better than the unshifted 5.308 and 0.1324** — the shift improves shoulder deformation rather than degrading it. Three deltoid origins re-anchored to the clavicle's moved lateral end (without which `deltoid_medial` went back to lengthening at 1.0045 in its own abduction, the exact defect its own comment records), and the pectoralis and latissimus insertions brought from 14 mm to 4 mm off the humerus shaft, a translation smaller than the widening, where the pec actually inserts.

That left 297 passed / 1 failed: `pectoralis_l` 2.9 mm outside the skin in the press. No non-scaling lever closed it — `taper` migrates the violation between poses, `flatten` has no effect on the probed depth, an explicit `outward` moves it under 0.3 mm, and the skin band does not change the figure at all because the probe reads the rig-derived profile skin rather than the baked surface.

**The blocker is the character candidate, not the re-derivations.** At curl Peak the Stage 1 character's hand tracks the canonical pose to about 10 mm; the Stage 2 candidate's sits 281 mm low and 123 mm forward — the arm hangs instead of curling — so the grip readings (fingers 2.4–94.1 mm out, thumb 440 mm, wrap 100°) and the +140 mm thigh figure are meaningless rather than poor. Metadata is intact on both candidates, so no grip data was lost. The cause looks structural in the method: translating `DEF-upper_arm.*` rigidly rotates the *clavicle*, whose head is at the sternum and whose tail is the moved child, and the change of basis `bindRetarget` computes once from the bind pose does not survive that. Stage 1's forearm tool used the same mechanism safely because it translated **along** a bone axis, leaving every bone direction unchanged; a lateral translation does not. Fixing it means expressing the widening as a clavicle rotation plus length rather than a rigid translation of its child.

Reverted rather than left in place: the canonical half is only correct together with a matching character, and shipping it against an arm 28 cm out would leave rig and character disagreeing about the shoulder, which the derivation notes is worse than either. Every value and reason is recorded, so re-applying is mechanical once the candidate is rebuilt. Phases 3–5 not started.
...
Each joint now advances along the authored pattern, proximal and distal together, and locks where that digit's own skinned surface first reaches the real finite cylinder. Solving one joint at a time from the base instead leaves the distal joints straight while the base rotates, so an extended finger sweeps the handle and locks the base far too early — that produced a pinky with a straight middle joint and a bent tip, which is not a shape a hand makes. Result at the frozen grip centre: zero digits inside (worst −0.46 mm of skin press), palm loaded at −1.14 mm, thumb opposing at +3.80 mm, **wrap 240°** — against ~160° for the only clean static profiles and −11.48 mm over 53 vertices for the authored one. The thumb needed no flexion change; its whole problem was the sign of `thumbOppositionX`, which takes no per-side flip.

Placed character-side, not in the grip profile. Putting the solved angles in `GRIP_PROFILES.dumbbell` broke four canonical tests (`index_02_l` 42.7 mm from the canonical handle against a 32 mm bound): the canonical rig has its own finger lengths and handle offset, so one angle table cannot serve both. The canonical rig keeps the authored profile; `src/character/solvedGrip.ts` holds the solution keyed by character and grip family with its solved radius, and `applyCharacterPose` substitutes it when the frame says what is held, never writing to the shared pose. `CharacterPoseContext.grip` carries `{kind, closure}`, `StudioClip.hands` carries the clip's grip, and `gripSolutionId` is only defaulted for a character that also carries its own `handleGripOffsets`. `bar`, `handle` and `rope` keep the shared −14 until solved in their own right.

Verified on the shipped path, not the harness's: `applyCharacterPose` with the grip context gives 4 vertices inside against 75 without it. One table re-used unchanged across curl Bottom/Mid/Peak, both hands and shoulder press gives identical figures — checked for deadness by perturbing the index MCP +12°, which moves it to −4.30 mm with 2 inside. New `solvedGrip.test.ts` guards the data, including that every solved angle is inside the rig's limits at the curl's closure, since the substitution does not re-run `clampPose`. Full suite 297 passed / 1 skipped / 1 failed (pre-existing `strainReview` timeout). Harness note: Vite reserves `PROD` and statically substitutes `process.env.PROD`, so a flag by that name reads empty inside a test — one run measured the wrong path before this surfaced; it is now `SHIPPED`.

**Not locked.** Criteria 1 and 3 are visual and I will not sign them off on a flat-shaded diagnostic render. It shows a genuine wrap — all four digits contacting, palm loaded, thumb opposing, not the ~160° cupped hand the decision rules out — but also ~120° of circumference open at the lower right, where the fingertips come around the handle without closing under it. Every MCP locks early (index 30.4° against an authored 66.3°) because the proximal phalanx reaches the handle almost immediately: the handle centre sits close enough to the MCP row that the knuckles cannot roll over it, capping wrap near 240°. That points at MCP placement against the frozen `handleGripOffsets`, a listed stop condition, so this is the report rather than the lock.

### Claude — 2026-09-16 — Phase 1: grip harness repaired; corrected metric inverts the earlier finding

Diagnostic repair only, no profile tuned or retained, recorded in `docs/REFERENCE_BODY_PHASE1_GRIP_METRIC.md`. Four faults fixed: vertex classification now assigns by **summed** group weight instead of taking the single highest-weighted bone and discarding vertices under 0.5 (a finger vertex splits ~0.45/0.40 across two phalanges, so those were dropped or miscounted as palm — the whole dominant-vs-summed disagreement); contact is now the signed distance to the real **capped** cylinder instead of an infinite one with an axial `continue`; the slab is no longer an env var (±40 in one harness, ±60 in another, which is why they disagreed) but the bar's real 60 mm half-length, with the bar axis checked against `geometry.ts` rather than assumed; and the sweep now regenerates the clip after each profile mutation and reports a posed-vertex checksum per row with a `SWEEP-DEAD` flag, so a number that cannot be traced to a moved mesh is not reported as a measurement. Six profiles now give six distinct checksums and a monotonic response; the metric is pose-invariant across Bottom/Mid/Peak as a rigidly held handle must be; renders share the metric's vertex set exactly (75 = 75); left and right agree to the digit.

**The corrected metric inverts the previous conclusion.** It is the fingers that penetrate — 11.48 mm over 53 vertices — and the palm that barely touches at −0.59 mm over one vertex, which is the firm contact a loaded grip should show. The earlier "4.61 mm / 2 vertices" finger figure was the misclassification, and the "palm is closure-invariant so closure cannot help" conclusion that followed from it is withdrawn. The rendered evidence agrees: looking down the bar axis, the bar's circle is entirely filled with hand surface — the fist closes solid, through the bar's volume.

Found and deliberately **not** retained: `thumbOppositionX` is written to `thumb_01.x` with no per-side sign flip, and the shipped `−14` rotates the thumb *into* the bar. `+14` clears it completely (−10.43/21 in → +2.54/0 in) with wrap unchanged and no effect on fingers or palm, identically on both hands. It is shared by every grip profile (`bar`, `handle`, `rope` all carry −14), so flipping it moves the thumb on pull-up, press and row too — that belongs in the Phase 3 validation pass, and it does not on its own complete Phase 1.

**Blocker.** No profile and no placement satisfies both acceptance criteria. 120 per-joint profiles searched: four clean, all `[25,25,*]` barely-flexed hands wrapping only ≈160°; the best wrap anywhere, 336°, costs 10.52 mm of penetration. With the full-fist profile, 522 of 6561 bar centres over ±40 mm clear the fingers and thumb and **none** keep the palm loaded — the bar can only avoid the fingers by leaving the hand. Root cause: `applyGrip` is a static angle table scaled by one closure scalar, so the fingers keep closing after they reach the bar; a cylindrical grip needs close-until-contact per digit, which is a solver change beyond "tune a profile" and needs authorising. Caveat stated on my own metric: wrap is only meaningful for a non-penetrating configuration, since a solid fist trivially scores high.
