"""Candidate-only V15a finger-shaft reconstruction in the prepared Blender file.
...
BLEND = ROOT / "HomeGymPT_Male_HIGH_DETAIL_CANDIDATE_v15a_deep_hand_rebuild.blend"
REPORT = ROOT / "reports" / "sculpt_v15a_digit_cores.json"
...
for key, core in all_cores.items():
    ...
    for edge in list(bm.edges):
        ...
        if len(f1.verts) != 3 or len(f2.verts) != 3 or f1.normal.dot(f2.normal) < 0.85:
            continue
        ...
        if gain < 0.28 or (opp[1].co - opp[0].co).length > edge.calc_length() * 1.65:
            continue
        candidates.append((gain, edge))
    local = 0
    used = set()
    for _, edge in sorted(candidates, key=lambda row: row[0], reverse=True):
        if local >= 70 or not edge.is_valid or any(f in used for f in edge.link_faces):
            continue
...
REPORT.write_text(json.dumps({"digits": results, "total_edge_flips": flips}, indent=2))
print("V15_SCULPT", json.dumps({"digits": results, "total_edge_flips": flips}), flush=True)
