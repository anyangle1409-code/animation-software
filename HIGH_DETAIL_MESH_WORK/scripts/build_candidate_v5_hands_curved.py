...
version=args[1] if len(args)>1 else 'v5_hands_curved'
loss_limit=.05
for side in ['L','R']:
...
# Keep every original hand position, normal, UV, colour and skin weight fixed.
# Refine only edges fully *inside* the hand; boundary edges remain unsplit so
# the forearm/wrist connection and retarget contact surface keep V4B vertices.
protected=feet;arrays={k:list(a) for k,a in attrs.items() if not k.startswith(('JOINTS_','WEIGHTS_'))};joints=list(ji);weights=list(we);mid={};lost=[]
contact_guard=np.zeros(N,dtype=bool)
contact_guard[json.loads((R/'reports/hand_contact_guard_v5.json').read_text())['original_vertex_ids']]=True
def midpoint(a,b):
 key=tuple(sorted((int(a),int(b))))
 if protected[a] or protected[b] or hand[a]!=hand[b] or ((hand[a] or hand[b]) and (contact_guard[a] or contact_guard[b])):return None
