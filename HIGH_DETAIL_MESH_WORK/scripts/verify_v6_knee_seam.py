"""Check V6 knee seam isolation, exact scope, and bare/dressed equivalence."""
...
v6_file = root / "HomeGymPT_Male_HIGH_DETAIL_CANDIDATE_v6_knee_seam.glb"
bare_file = root / "HomeGymPT_Male_HIGH_DETAIL_CANDIDATE_v6_knee_seam_BARE.glb"
...
v6, v6_bin = glb(v6_file)
bare, bare_bin = glb(bare_file)
assert v6_bin[: len(source_bin)] == source_bin
assert v6_bin == bare_bin
assert source["meshes"][1:] == v6["meshes"][1:]
for field in ("nodes", "skins", "scenes", "animations", "materials", "textures", "images", "samplers"):
    assert v6.get(field) == source.get(field), field
expected_bare = json.loads(json.dumps(v6))
...
v6_primitive = v6["meshes"][0]["primitives"][0]
...
for attr in source_primitive["attributes"]:
    if attr in ('POSITION', 'NORMAL'):
        continue
    old = read(v5, v5_bin, v5_primitive["attributes"][attr])
    new = read(v6, v6_bin, v6_primitive["attributes"][attr])
    assert np.array_equal(old, new), attr
seams = json.loads((root / 'reports/knee_seam_v5.json').read_text())['knees']
allowed = {index for entry in seams.values() for path in entry['paths'] for index in path}
for attr in ('POSITION', 'NORMAL'):
    old = read(v5, v5_bin, v5_primitive['attributes'][attr])
    new = read(v6, v6_bin, v6_primitive['attributes'][attr])
    changed = set(np.flatnonzero(np.any(old != new, axis=1)))
    assert changed <= allowed, (attr, changed - allowed)

vertices = read(v6, v6_bin, v6_primitive["attributes"]["POSITION"]).astype(float)
triangles = read(v6, v6_bin, v6_primitive["indices"]).reshape(-1, 3)
assert np.array_equal(triangles, read(v5, v5_bin, v5_primitive['indices']).reshape(-1, 3))
...
prior = json.loads((root / "reports/final_integrity_v5_hands_curved.json").read_text())["artifacts"]
assert all(digest(root / name) == value["sha256"] for name, value in prior.items())
report = {
    "reference_files_unchanged": len(hashes),
    "v5_artifacts_unchanged": True,
    "source_rig_skin_animation_and_materials_equal": True,
    "v5_all_nonposition_attributes_equal": True,
    "changed_vertices_limited_to_knee_paths": True,
    ...
    "artifacts": {path.name: {"bytes": path.stat().st_size, "sha256": digest(path)} for path in (v6_file, bare_file, root / "HomeGymPT_Male_HIGH_DETAIL_CANDIDATE_v6_knee_seam.blend")},
}
(root / "reports/final_integrity_v6_knee_seam.json").write_text(json.dumps(report, indent=2))
