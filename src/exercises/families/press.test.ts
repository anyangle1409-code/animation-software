...
describe('the press family', () => {
  ...
  it('has exactly two registered variants, which the header explains', () => {
    ...
    // and the bench press lies down, in the supine family.
    const presses = EXERCISES.filter(
      (exercise) =>
        exercise.clipName.includes('press') && exercise.category !== 'core' && !exercise.clipName.includes('bench'),
    );
