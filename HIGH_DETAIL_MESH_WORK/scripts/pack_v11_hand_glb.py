"""Transplant the V11 Blender hand cleanup into the accepted V8 GLB container.

The V10 packer (`pack_v10_hand_glb.py`) with two changes: the source Blend and
target GLB come from the environment (`BLEND`, `TARGET`), and vertices that
share a position in the hand region share one normal. The hand surface is not
a welded sheet - its seams are closed by moving vertices, not merging them - so
without this each side of a closed seam would still shade separately.
...
BLEND=Path(os.environ.get('BLEND',ROOT/f'HomeGymPT_Male_HIGH_DETAIL_CANDIDATE_{VERSION}.blend'))
TARGET=Path(os.environ.get('TARGET',ROOT/f'HomeGymPT_Male_HIGH_DETAIL_CANDIDATE_{VERSION}.glb'))
REPORT=Path(os.environ.get('REPORT',ROOT/f'reports/build_{VERSION}_glb.json'))
if TARGET.exists():raise SystemExit('Refusing to overwrite an existing GLB')
...
seam_attr=mesh.attributes.get('v11_seam_weights')
seam_weighted=np.asarray([d.value for d in seam_attr.data]) if seam_attr else np.zeros(len(mesh.vertices),int)
...
# Original seam vertices the build re-weighted to match their partner surface
# (`v11_seam_weights`) take their Blender weights; every other original keeps V8's.
joints_arr={lane:old[f'JOINTS_{lane}'].copy() for lane in range(3)}
weights_arr={lane:old[f'WEIGHTS_{lane}'].copy() for lane in range(3)}
original_reweighted=0
for v in mesh.vertices:
    sid=source_ids[v.index]
    if sid<0 or not seam_weighted[v.index]:continue
    joints,weights,loss=skin_record(v)
    for gi in variants[int(sid)]:
        for lane in range(3):
            joints_arr[lane][gi]=joints[lane*4:(lane+1)*4]
            weights_arr[lane][gi]=weights[lane*4:(lane+1)*4]
    original_reweighted+=1
for lane in range(3):
    old[f'JOINTS_{lane}']=joints_arr[lane];old[f'WEIGHTS_{lane}']=weights_arr[lane]
...
# One normal per position across the hand region's seams (GLB frame: x is
# sideways, y is up). Area-weighted face normals, so a sliver does not tip it.
P=updated['POSITION'].astype(float);Nn=updated['NORMAL'].astype(float)
region=(np.abs(P[:,0])>.55)&(P[:,1]>1.0)&(P[:,1]<1.4)
face_n=np.zeros_like(P)
F=np.asarray(tri,dtype=int)
cross=np.cross(P[F[:,1]]-P[F[:,0]],P[F[:,2]]-P[F[:,0]])
for k in range(3):np.add.at(face_n,F[:,k],cross)
keys=defaultdict(list)
for i in np.flatnonzero(region):keys[tuple(np.round(P[i]*1e5).astype(np.int64))].append(i)
shared=0
for members in keys.values():
    if len(members)<2:continue
    total=face_n[members].sum(axis=0);length=np.linalg.norm(total)
    if length<1e-15:continue
    Nn[members]=total/length;shared+=len(members)
updated['NORMAL']=Nn.astype(updated['NORMAL'].dtype)
...
report['seam_vertices_sharing_normals']=shared
report['original_seam_vertices_reweighted']=original_reweighted
REPORT.parent.mkdir(parents=True,exist_ok=True)
REPORT.write_text(json.dumps(report,indent=2))
print('V11_HAND_GLB',json.dumps(report),flush=True)
