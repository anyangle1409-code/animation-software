"""Audit influence loss on V5 contact-safe hand and body midpoints."""
...
protected = feet
contact_guard = np.zeros(count, dtype=bool)
contact_guard[json.loads((ROOT / 'reports/hand_contact_guard_v5.json').read_text())['original_vertex_ids']] = True
...
def midpoint(a, b):
    a, b = int(a), int(b)
    if protected[a] or protected[b] or hand[a] != hand[b] or ((hand[a] or hand[b]) and (contact_guard[a] or contact_guard[b])):
        return
