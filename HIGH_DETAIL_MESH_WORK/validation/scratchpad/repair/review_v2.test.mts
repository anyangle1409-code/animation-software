...
const dest='../reports/poses_v2';mkdirSync(dest,{recursive:true});
const sources={baseline:'../reference/anyangle1409-code-animation-software-f9cca7c/HOME_GYM_PT_GPT_MESH_HANDOFF/characters/HomeGymPT_Male_CORNER_FINAL_SHORTS.glb',candidate:'../HomeGymPT_Male_HIGH_DETAIL_CANDIDATE_v2.glb'};
it('compares all five exercises with frozen rig, correctives and actual equipment frames',async()=>{
 ...
 const frames=[['bottom',0],...Array.from({length:23},(_,i)=>[`sample_${i+1}`,clip.duration*(i+1)/24]),['peak',peak],['return',clip.duration]] as [string,number][];let maxHand=0,maxBones=0,maxEquipment=0;const start:any={};
 ...
 writeFileSync('../reports/exercise_validation_v2.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report.exercises));Object.values(builds).forEach((c:any)=>c.dispose());
},900000);
