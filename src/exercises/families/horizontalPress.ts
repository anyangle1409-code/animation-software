 ...
 * away from the chest, with the arms working at right angles to the trunk.
 *
 * The push-up was the family's first exercise, written out in full and accepted
 * before families existed; it is re-expressed here without changing a value, so
 * `pushUp` is byte-identical to the accepted clip (`horizontalPress.test.ts`).
 * What a variant may change is where the hands are planted: the toes, the root
 * placement and the elbow poles are measured for the standard hand position
 * and stay with it until a variant is measured on its own.
 *
...
export interface HorizontalPressVariant {
  id: string;
  name: string;
  clipName: string;
  description: string;
  /** Left hand's planted position; the right is its mirror. The standard hand when absent. */
  hand?: Vec3;
  tempo?: Tempo;
...
export function horizontalPressFamily(variant: HorizontalPressVariant): ExerciseDefinition {
  const hand = variant.hand ?? HAND_L;
  return {
    id: variant.id,
    name: variant.name,
    ...
    tempo: variant.tempo ?? { eccentric: 1.6, pauseStretched: 0.4, concentric: 1.2, pauseContracted: 0.3 },
