...
const version='v10_hand_retopology';
const dest='../reports/poses_'+version;
const files={baseline:'../HomeGymPT_Male_HIGH_DETAIL_CANDIDATE_v8_knee_anatomy.glb',
...
it('captures equipment-free open hand and closed fist review poses with the frozen runtime',async()=>{
 ...
 expect(report.open_hand_baseline.meshVertices[0]).toBe(33089);
 expect(report.open_hand_candidate.meshVertices[0]).toBeGreaterThan(33089);
 writeFileSync('../reports/v10_hand_studies.json',JSON.stringify(report,null,2));
},900000);
