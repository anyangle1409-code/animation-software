...
sheet(
    "V4B_OVERHEAD_REVISIONS.jpg",
    [
        (f"{exercise} {who} - three-quarter",
         (root / ('renders_v3' if who in ('Frozen', 'V3') else 'renders_v4b') /
          f"{prefix}_peak_{'baseline' if who == 'Frozen' else 'candidate'}_shoulder_three_quarter.png"))
        for exercise, prefix in (("Shoulder press", "dumbbell_shoulder_press"), ("Pull-up", "pull_up"))
        for who in ("Frozen", "V3", "V4B")
    ],
    columns=3,
    cell_size=(420, 460),
)
