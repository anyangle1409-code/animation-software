...
gain=float(sys.argv[sys.argv.index('--')+1]) if '--' in sys.argv else 1.0
for side in ['L','R']:
 ...
 mask=height*lateral*depth*ownership
 world=np.zeros_like(pos);world[:,1]=.027*mask*gain;world[:,2]=-.040*mask*gain
...
region=np.zeros(N)
for side in ['L','R']:
 rel=posed-np.array(ref['shoulders'][side]);region=np.maximum(region,np.exp(-((rel[:,0]/.095)**4+(rel[:,1]/.115)**4+(rel[:,2]/.12)**4)))
region[region<.02]=0
for _ in range(5):
...
def midpoint(a,b):
 ...
 ranked=sorted(totals.items(),key=lambda x:-x[1]);chosen=ranked[:4];total=sum(x[1] for x in chosen);lost.append(max(0,sum(totals.values())-total));chosen+= [(0,0)]*(4-len(chosen));joints.append([x[0] for x in chosen]);weights.append([x[1]/total for x in chosen])
...
dest=R/'HomeGymPT_Male_HIGH_DETAIL_CANDIDATE_v1.glb';dest.write_bytes(glb)
report={'body_triangles':len(newtri),'source_body_triangles':len(tri),'vertices':len(joints),'original_vertices':N,'original_hand_vertices_untouched':int(hand.sum()),'sculpt_vertices':int(affected.sum()),'max_sculpt_mm':float(np.linalg.norm(delta,axis=1).max()*1000),'new_vertex_weight_loss_max':max(lost),'new_vertex_weight_loss_mean':sum(lost)/len(lost),'gain':gain,'source_rig_and_metadata_equal':all(j.get(k)==out.get(k) for k in ['nodes','skins','scenes','animations'])}
(R/'reports/build.json').write_text(json.dumps(report,indent=2));print(json.dumps(report),flush=True)
...
bpy.ops.wm.save_as_mainfile(filepath=str(R/'HomeGymPT_Male_HIGH_DETAIL_CANDIDATE_v1.blend'))
bpy.ops.wm.save_as_mainfile(filepath=str(R/f'checkpoints/CANDIDATE_v1_03_surface_gain_{gain:.2f}.blend'),copy=True)
