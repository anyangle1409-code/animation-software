...
## Fixed since the freeze (no structural change)

- **Mirrored characters: hand roll.** The hand, thumb and finger frames carry a small roll from the rig's knuckle plane to its anatomical frame (2.77° for the hand, 2.75–2.80° for the fingers, 4.31° for the thumb base). That roll is a rig-space rotation and was not reflected for a mirrored character, which turned it the wrong way: an error of twice the angle. So the knuckle fan sat 5.5° off (6.2° in the curls, 8.6° at the thumb base), against 0.5° same-side. It is now reflected like every other canonical frame (`src/retargeting/retarget.ts`), and mirrored and same-side characters agree to arccos resolution (`movementCertification.test.ts`, `mirroredHands.test.ts`). Grip offsets embedded in older assets were measured in the old frame and are carried into the new one (`legacyHandFrame`), unless the asset declares `offsetFrame: "hand-v2"`. No bone, rest transform, exercise or export track changed.
