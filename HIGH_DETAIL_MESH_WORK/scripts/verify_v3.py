"""Check v3 candidate isolation, geometry and bare/dressed equivalence."""
...
v2_file = root / "HomeGymPT_Male_HIGH_DETAIL_CANDIDATE_v2.glb"
v3_file = root / "HomeGymPT_Male_HIGH_DETAIL_CANDIDATE_v3.glb"
bare_file = root / "HomeGymPT_Male_HIGH_DETAIL_CANDIDATE_v3_BARE.glb"
...
v2, v2_bin = glb(v2_file)
v3, v3_bin = glb(v3_file)
bare, bare_bin = glb(bare_file)
assert v3_bin[: len(source_bin)] == source_bin
assert v3_bin == bare_bin
assert source["meshes"][1:] == v3["meshes"][1:]
for field in ("nodes", "skins", "scenes", "animations", "materials", "textures", "images", "samplers"):
    assert v3.get(field) == source.get(field), field
expected_bare = json.loads(json.dumps(v3))
...
v2_primitive = v2["meshes"][0]["primitives"][0]
...
for attr in source_primitive["attributes"]:
    old = read(v2, v2_bin, v2_primitive["attributes"][attr])[:source_count]
    new = read(v3, v3_bin, v3_primitive["attributes"][attr])[:source_count]
    assert np.array_equal(old, new), attr

vertices = read(v3, v3_bin, v3_primitive["attributes"]["POSITION"]).astype(float)
triangles = read(v3, v3_bin, v3_primitive["indices"]).reshape(-1, 3)
...
prior = json.loads((root / "reports/final_integrity_v2.json").read_text())["artifacts"]
...
report = {
    ...
    "v2_original_vertices_and_attributes_equal": True,
    ...
    "artifacts": {path.name: {"bytes": path.stat().st_size, "sha256": digest(path)} for path in (v3_file, bare_file, root / "HomeGymPT_Male_HIGH_DETAIL_CANDIDATE_v3.blend")},
}
(root / "reports/final_integrity_v3.json").write_text(json.dumps(report, indent=2))
