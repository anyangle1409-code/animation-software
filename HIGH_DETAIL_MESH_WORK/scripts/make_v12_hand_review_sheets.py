...
sets={
 f'{label.upper()}_V11_OPEN_HAND_COMPARISON.jpg':[('Open palm','open_hand_review','hand'),('Back','open_hand_review','hand_back'),('Thumb-index web','open_hand_review','hand_web')],
 f'{label.upper()}_V11_CLOSED_FIST_COMPARISON.jpg':[('Palm','closed_fist_review','hand'),('Back','closed_fist_review','hand_back'),('Side','closed_fist_review','hand_side')],
 f'{label.upper()}_V11_EXERCISE_HAND_COMPARISON.jpg':[('Curl grip','dumbbell_bicep_curl_bottom','hand'),('Push-up contact','push_up_bottom','hand'),('Pull-up grip','pull_up_peak','hand')],
}
for name,views in sets.items():
    ...
    for col,(title,pose,view) in enumerate(views):
        ...
        for row,kind in enumerate(('baseline','candidate')):
            ...
            draw.text((col*w+12,46+row*h),'V11' if row==0 else label,fill=(255,255,255),font=font,stroke_width=2,stroke_fill=(0,0,0))
