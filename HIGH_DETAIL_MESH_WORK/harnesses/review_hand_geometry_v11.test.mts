...
// V11: the gate compares against accepted V8 (as V10's did); the review boards
// are rendered against V10 by pointing BASELINE_GLB at it.
const sources={baseline:process.env.BASELINE_GLB??'../HomeGymPT_Male_HIGH_DETAIL_CANDIDATE_v8_knee_anatomy.glb',candidate:`../HomeGymPT_Male_HIGH_DETAIL_CANDIDATE_${version}.glb`};
const reportName=process.env.REPORT_NAME??`exercise_validation_${version}`;
it('compares all five exercises with frozen rig, correctives and actual equipment frames',async()=>{
 ...
 if(ex.id==='dumbbell_bicep_curl'&&label==='bottom'&&id==='baseline'){
  ...
  if(!process.env.BASELINE_GLB)writeFileSync(`../reports/${version}_actual_curl_skin_matrices.json`,JSON.stringify({boneNames:body.skeleton.bones.map((b:any)=>b.name),matrices}));
 }
 ...
 writeFileSync(`../reports/${reportName}.json`,JSON.stringify(report,null,2));console.log(JSON.stringify(report.exercises));Object.values(builds).forEach((c:any)=>c.dispose());
},900000);
