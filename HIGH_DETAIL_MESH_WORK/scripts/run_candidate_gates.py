"""Run candidate-only validation against the confirmed 55-bone source baseline.

Run from HIGH_DETAIL_MESH_WORK:
    python scripts/run_candidate_gates.py --version v7_example

The exact source commit is c2372c1 (hgpt_canonical_v2). The source is extracted
into validation_55 without merging it into the mesh-review branch.
"""
...
def ensure_rig55_source():
    marker=VALIDATION/".rig_source_commit"
    if not marker.is_file() or marker.read_text().strip()!=RIG_SHA:
        run([sys.executable,ROOT/"scripts"/"prepare_rig55_validation.py"],cwd=ROOT)
    if not (VALIDATION/"src"/"rig"/"scapula.test.ts").is_file():
        raise SystemExit("validation_55 is not the confirmed 55-bone source")
...
def vitest_command():
    ...
    raise SystemExit("Vitest is unavailable in validation_55")
...
def main():
    ...
    for name in GUARDS+["vitest.config.mts","review_v3.test.mts"]:
        if not (REPAIR/name).is_file():raise SystemExit(f"Missing rig-55 harness: {REPAIR/name}")
    ...
    run(runner+["--config",str(config.relative_to(VALIDATION)),*guard_files],
        cwd=VALIDATION,env=env,log=ROOT/"reports"/f"{version}_rig55_guards.log")
    ...
    if not args.skip_exercise:
        ...
        run(runner+["--config",str(config.relative_to(VALIDATION)),review],
            cwd=VALIDATION,env=env,log=ROOT/"reports"/f"{version}_rig55_exercises.log")

    print("\nCANDIDATE GATES PASS AGAINST",RIG_SHA)
    print("skeleton: hgpt_canonical_v2 / 55 bones")
    print("guards:",ROOT/"reports"/f"{version}_rig55_guards.log")
    if not args.skip_exercise:print("exercises:",ROOT/"reports"/f"{version}_rig55_exercises.log")
