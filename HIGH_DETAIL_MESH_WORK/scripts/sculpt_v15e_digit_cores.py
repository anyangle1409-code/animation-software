"""Candidate-only V15e finger-shaft reconstruction in the prepared Blender file.
...
BLEND = ROOT / "HomeGymPT_Male_HIGH_DETAIL_CANDIDATE_v15e_deep_hand_rebuild.blend"
REPORT = ROOT / "reports" / "sculpt_v15e_digit_cores.json"
...
for key, core in all_cores.items():
    ...
    results[key]["joint_control_edges_split"] = len(edges)
    selected_edges.extend(edges)
operation = bmesh.ops.subdivide_edges(bm, edges=selected_edges, cuts=1, use_grid_fill=False)
